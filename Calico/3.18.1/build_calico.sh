...
PACKAGE_NAME="calico"
PACKAGE_VERSION="v3.18.1"
ETCD_VERSION="v3.3.7"
GOLANG_VERSION="go1.15.2.linux-s390x.tar.gz"
FORCE="false"
TESTS="false"
CURDIR="$(pwd)"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.18.1/patch"
...
EOF
    ...
    # Build go-build v0.49
    rm -rf $GOPATH/src/github.com/projectcalico/go-build
    git clone -b v0.49 https://github.com/projectcalico/go-build $GOPATH/src/github.com/projectcalico/go-build 2>&1 | tee -a "$GOBUILD_LOG"
    cd $GOPATH/src/github.com/projectcalico/go-build
    printf -- "\nApplying patch for go-build Makefile ... \n" | tee -a "$GOBUILD_LOG"
    curl -s $PATCH_URL/Dockerfile.s390x.v0.49 | git apply - 2>&1 | tee -a "$GOBUILD_LOG"
    # Then build `calico/go-build-s390x:v0.49` image
    ARCH=s390x VERSION=v0.49 ARCHIMAGE='$(DEFAULTIMAGE)' make image | tee -a "$GOBUILD_LOG"
    ...
    git clone -b v3.19.0-0.dev https://github.com/projectcalico/libcalico-go $GOPATH/src/github.com/projectcalico/libcalico-go
    git clone -b v3.18.0-0.dev https://github.com/projectcalico/confd $GOPATH/src/github.com/projectcalico/confd
    ...
    curl -s $PATCH_URL/kube-ctl.patch | git apply - 2>&1 | tee -a "$KCTL_LOG"
...
EOF
    ...
    cd $GOPATH/src/github.com/projectcalico/felix
    ARCH=s390x CALICOCTL_VER=latest CNI_VER=latest-s390x EXTRA_DOCKER_ARGS="-v $(pwd)/../:/go/src/github.com/projectcalico" make test 2>&1 | tee -a "$TEST_FELIX_LOG"
